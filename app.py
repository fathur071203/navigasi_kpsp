...
for item in TOOLS:
	...
	if item["url"]:
		button_html.append(
			f'<a class="nav-btn {kind_class}" href="{item["url"]}" target="_self">{label}</a>'
		)
	else:
...
st.markdown(
	'<div class="note">Logo memakai file static/Logo.png dan seluruh navigasi tetap pada halaman yang sama.</div>',
	unsafe_allow_html=True,
)
